...
from .models import AdminAuditLog, Dispute, Payment, PrestataireAvailabilitySlot, PrestataireUnavailability, Provider, Reservation
...
def send_babifix_email(to_email: str, subject: str, body: str) -> None:
    ...
    if 'console' in backend:
        logger.warning('EMAIL_BACKEND=console → email NON envoyé à %s (sujet: %s). Configurez SMTP dans les variables d\'environnement Render.', to_email, subject)
        return
    try:
        send_mail(
            subject=subject,
        ...
        logger.error('ÉCHEC envoi email %s (sujet: %s) : %s', to_email, subject, exc)
...
def email_welcome(user, role: str) -> None:
    ...
    body += (
        'L\'équipe BABIFIX — Côte d\'Ivoire\n'
...
def api_prestataire_availability(request):
    ...
    if not isinstance(is_available, bool):
        return JsonResponse({'error': 'is_available (or disponible) must be a boolean'}, status=400)
...
def api_admin_audit_log(request):
    ...
    logs = list(
        qs[(page - 1) * per_page: page * per_page].values(
            ...
            'details', 'created_at', 'admin_user__username',
        )
    )
...
def api_admin_export_csv(request, kind):
    ...
    if kind == 'reservations':
        ...
        headers = [
            'id', 'reference', 'client', 'prestataire', 'montant',
            'statut', 'cash_flow_status', 'payment_type',
            'mobile_money_operator', 'address_label',
        ]
        def row_iter():
            w = csv.writer(writer)
            ...
            for r in qs.iterator():
                yield w.writerow([
                    r.pk, r.reference, r.client, r.prestataire, r.montant,
                    r.statut, r.cash_flow_status, r.payment_type,
                    r.mobile_money_operator, r.address_label,
                ])
    elif kind == 'prestataires':
        qs = Provider.objects.all()
        headers = [
            'id', 'nom', 'specialite', 'ville', 'statut', 'disponible',
            'tarif_horaire', 'average_rating', 'rating_count', 'is_approved',
        ]
        def row_iter():
            w = csv.writer(writer)
            yield w.writerow(headers)
            for p in qs.iterator():
                yield w.writerow([
                    p.pk, p.nom, p.specialite, p.ville, p.statut, p.disponible,
                    p.tarif_horaire, p.average_rating, p.rating_count, p.is_approved,
                ])
    elif kind == 'paiements':
        qs = Payment.objects.all()
        headers = [
            'id', 'reference', 'client', 'prestataire', 'montant',
            'etat', 'type_paiement', 'valide_par_admin', 'reference_externe',
        ]
        def row_iter():
            w = csv.writer(writer)
            yield w.writerow(headers)
            for p in qs.iterator():
                yield w.writerow([
                    p.pk, p.reference, p.client, p.prestataire, p.montant,
                    p.etat, p.type_paiement, p.valide_par_admin, p.reference_externe,
                ])
    else:
        return JsonResponse({'error': f'kind inconnu : {kind}'}, status=400)

    response = StreamingHttpResponse(row_iter(), content_type='text/csv; charset=utf-8-sig')
...
def api_prestataire_availability_crud(request):
    ...
    if request.method == 'GET':
        slots = PrestataireAvailabilitySlot.objects.filter(provider=provider, actif=True)
        data = [
            {
                'id': s.id,
        ...
        if not all([jour, debut, fin]):
            return JsonResponse({'error': 'jour_semaine, heure_debut, heure_fin required'}, status=400)
        ...
        if slot_id:
            PrestataireAvailabilitySlot.objects.filter(pk=int(slot_id), provider=provider).delete()
...
def api_prestataire_unavailability_crud(request):
    ...
    if request.method == 'GET':
        ...
        if period_id:
            PrestataireUnavailability.objects.filter(pk=int(period_id), provider=provider).delete()
            return JsonResponse({'ok': True})
        return JsonResponse({'error': 'id required'}, status=400)
...
def api_prestataire_stats(request):
    ...
    en_cours = reservations.filter(statut__in=[Reservation.Status.CONFIRMED, Reservation.Status.IN_PROGRESS]).count()
...
def api_client_favorites(request):
    ...
    if request.method == 'GET':
        ...
        favorites = ClientFavorite.objects.filter(client=client_user).select_related('provider')
        data = [
            {
                'id': f.provider.id,
                'nom': f.provider.nom,
        ...
        try:
            provider = Provider.objects.get(id=provider_id, statut=Provider.Status.VALID)
        except Provider.DoesNotExist:
            return JsonResponse({'error': 'provider_not_found'}, status=404)
...
def send_booking_done_email(reservation: 'Reservation') -> None:
    ...
    html_content = _render_email_template('booking_done.html', {
...
def send_dispute_opened_email(
    ...
    html_content = _render_email_template('dispute_opened.html', {
...
def send_newsletter_confirmation_email(email: str, confirm_url: str) -> None:
    ...
    send_babifix_email_html(
        ...
        html_content=_render_email_template('newsletter_confirmation.html', {
...
def send_weekly_digest_email(prestataire: 'Provider', stats_dict: dict) -> None:
    ...
    send_babifix_email_html(
        ...
        html_content=_render_email_template('weekly_digest_prestataire.html', {
...
def send_babifix_email_html(to_email: str, subject: str, html_content: str) -> None:
    ...
    backend = getattr(settings, 'EMAIL_BACKEND', 'non configuré')
    if 'console' in backend:
        logger.warning('EMAIL_BACKEND=console → email HTML NON envoyé à %s (sujet: %s). Configurez SMTP.', to_email, subject)
        return
    try:
        if html_content:
        ...
        logger.error('ÉCHEC envoi email HTML %s (sujet: %s) : %s', to_email, subject, exc)
