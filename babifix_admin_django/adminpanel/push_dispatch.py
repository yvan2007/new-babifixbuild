...
def _schedule(user_ids: list[int | None], title: str, body: str, data: dict[str, Any] | None = None) -> None:
...
def _all_client_user_ids() -> list[int]:
    return list(
        UserProfile.objects.filter(role=UserProfile.Role.CLIENT, active=True).values_list('user_id', flat=True)
    )


def _schedule_all_clients_push(title: str, body: str, data: dict[str, Any] | None = None) -> None:
    _schedule(_all_client_user_ids(), title, body, data)


def _schedule_clients_and_prestataires_push(title: str, body: str, data: dict[str, Any] | None = None) -> None:
    ids = list(
...
def _should_broadcast_provider_to_clients(instance: Provider, created: bool, update_fields: frozenset[str] | None) -> bool:
    if instance.statut != Provider.Status.VALID:
        return False
    if created:
        return True
    if update_fields is not None and 'statut' in update_fields:
        return True
    return False
...
def on_reservation_change(instance, created: bool, update_fields: frozenset[str] | None) -> None:
...
def on_provider_change(instance, created: bool, update_fields: frozenset[str] | None) -> None:
    ...
    if instance.statut == 'Valide':
        ...
        body = reason if reason else 'Votre dossier a été refusé. Consultez le motif dans l’app.'
    else:
...
def on_actualite_published(instance, created: bool, update_fields: frozenset[str] | None) -> None:
...
def on_chat_message_created(instance: Message) -> None:
    ...
    try:
        if conv.reservation_id:
            ...
            r = Reservation.objects.filter(pk=conv.reservation_id).only('reference').first()
            if r:
                ref = r.reference
    except Exception:
        pass
    title = 'BABIFIX — Nouveau message'
    body = f'{peer_label} — réservation {ref}' if ref else f'Nouveau message de votre {peer_label.lower()}'
    data = {
        'type': 'chat.message',
...
def on_payment_change(instance, created: bool, update_fields: frozenset[str] | None) -> None:
    ...
    if instance.reservation_id:
        ...
        res = Reservation.objects.filter(pk=instance.reservation_id).only(
            'client_user_id', 'prestataire_user_id'
        ).first()
        if res:
