...
def cinetpay_webhook(request):
    ...
    if not _verify_webhook_signature(payload, received_sig):
        logger.warning('CinetPay webhook — signature invalide pour tx=%s', transaction_id)
    ...
    try:
        ...
        return JsonResponse({'message': 'OK'})  # Toujours 200 pour éviter les retry CinetPay
        ...
        try:
            ...
            if expected_amount != received_amount:
                logger.warning(
                    ...
                    transaction_id, expected_amount, received_amount
                )
                return JsonResponse({'error': 'amount_mismatch'}, status=400)
        except (TypeError, ValueError) as e:
            logger.warning('CinetPay webhook — montant invalide tx=%s: %s', transaction_id, e)
    ...
    if result_code == '00':
        ...
        logger.info('CinetPay webhook — paiement %s ÉCHEC (code=%s)', payment.reference, result_code)
