...
def _res_to_dict(res: Reservation, uid: int) -> dict:
    ...
    return {
        ...
        'can_dispute': res.statut in ('Terminee', 'En cours') and not res.dispute_ouverte,
        'rated': has_rating,
...
def api_client_reservations_list(request):
    ...
    qs = Reservation.objects.filter(client_user_id=uid).select_related(
        'assigned_provider', 'rating'
    ).order_by('-pk')
    if statut_filter:
...
def api_client_cancel_reservation(request, reference):
    ...
    if res.statut not in ('En attente', 'Confirmee'):
        return JsonResponse({'error': 'cannot_cancel', 'statut': res.statut}, status=400)
...
def api_client_open_dispute(request, reference):
    ...
    if res.statut not in ('Terminee', 'En cours'):
        return JsonResponse({'error': 'cannot_dispute', 'statut': res.statut}, status=400)
    try:
...
def api_auth_forgot_password(request):
    ...
    return JsonResponse({'ok': True, 'message': 'Si cet email existe, un lien a été envoyé.'})
...
def _send_verification_email(to_email: str, token: str) -> None:
    ...
    if 'console' in backend:
        logger.warning('EMAIL_BACKEND=console → vérification email NON envoyée à %s', to_email)
        return
    try:
        send_mail(
        ...
        logger.error('ÉCHEC envoi vérification email %s : %s', to_email, exc)
...
def _send_reset_email(to_email: str, token: str) -> None:
    ...
    backend = getattr(settings, 'EMAIL_BACKEND', 'non configuré')
    if 'console' in backend:
        logger.warning('EMAIL_BACKEND=console → reset email NON envoyé à %s', to_email)
        return
    ...
    try:
        ...
        logger.error('ÉCHEC envoi reset email %s : %s', to_email, exc)
...
def api_auth_reset_password(request):
    ...
    profile = UserProfile.objects.filter(reset_token=token).select_related('user').first()
    if not profile:
...
def api_auth_verify_email(request, token):
    profile = UserProfile.objects.filter(email_verify_token=token).select_related('user').first()
    if not profile:
        return JsonResponse({'error': 'invalid_token'}, status=400)
...
def api_prestataire_profile_update(request):
    ...
    if request.method == 'GET':
        return JsonResponse({
            ...
            'bio': provider.bio,
            'tarif_horaire': float(provider.tarif_horaire) if provider.tarif_horaire else None,
...
def api_user_notifications(request):
    ...
    return JsonResponse({'notifications': data, 'unread': sum(1 for n in data if not n['lu'])})
...
def api_admin_push_broadcast(request):
    ...
    target_role = str(payload.get('role', '') or '').strip().lower()  # '', 'client', 'prestataire'
