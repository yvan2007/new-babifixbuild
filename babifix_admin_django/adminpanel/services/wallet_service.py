...
def _get_effective_commission_rate(provider) -> Decimal:
    """Commission effective = barème dégressif par volume (ou catégorie) − réduction premium.

    Logique centralisée dans ProviderSubscriptionService pour rester cohérente
    avec l'affichage et le calculateur de rentabilité.
    """
    try:
        ...
        return max(Decimal("0.05"), Decimal(str(pct)) / Decimal("100"))
    except Exception:
        logger.exception("commission rate fallback")
        return Decimal("0.18")
