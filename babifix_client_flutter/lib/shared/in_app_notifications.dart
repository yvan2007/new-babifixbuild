...
class BabifixInAppNotif {
  ...
  factory BabifixInAppNotif.fromJson(Map<String, dynamic> j) {
    BabifixNotifAudience aud = BabifixNotifAudience.client;
    try {
      aud = BabifixNotifAudience.values.firstWhere(
        (e) => e.name == '${j['audience']}',
      );
    } catch (_) {}
    BabifixNotifSeverity sev = BabifixNotifSeverity.info;
    try {
      sev = BabifixNotifSeverity.values.firstWhere(
        (e) => e.name == '${j['severity']}',
      );
    } catch (_) {}
    DateTime at;
...
Future<void> persistInAppNotifHub(
  String storageKey,
  List<BabifixInAppNotif> list,
) async {
...
Future<List<BabifixInAppNotif>> loadInAppNotifList(String storageKey) async {
  ...
  try {
    ...
    return list
        .map(
          (e) =>
              BabifixInAppNotif.fromJson(Map<String, dynamic>.from(e as Map)),
        )
        .toList();
  } catch (_) {
    return [];
