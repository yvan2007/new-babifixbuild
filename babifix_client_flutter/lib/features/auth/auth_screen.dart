...
class _AuthScreenState extends State<AuthScreen>
  ...
  Future<void> _checkBiometricLogin() async {
    ...
    if (biometricEnabled && isLoggedIn) {
      ...
      if (isAvailable && mounted) {
        setState(() { _showBiometricLogin = true; _isLoading = false; });
        return;
      }
    }
    if (mounted) setState(() { _showBiometricLogin = false; _isLoading = false; });
  }
  ...
  Future<void> _doLogin() async {
    ...
    final err = await BabifixUserStore.login(emailCtrl.text.trim(), passCtrl.text);
    if (!mounted) return;
      ...
      MaterialPageRoute<void>(
        ...
        builder: (_) => EmailVerificationScreen(
          ...
          onContinue: () {
            ...
            showPostSignupOnboardingIfNeeded(context, onDone: widget.onAuthSuccess);
          },
        ),
      ),
    );
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          try {
                            ...
                            if (res.statusCode == 200) {
                              ...
                              _snack('Adresse email introuvable ou erreur serveur.');
                            }
                          } catch (_) {
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            gradient: const LinearGradient(colors: [_blue, _blueDeep]),
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return const Scaffold(
        ...
        body: Center(child: CircularProgressIndicator(color: Color(0xFF4CC9F0))),
      );
    }
        ...
        body: Stack(
            ...
            child: DecoratedBox(
              decoration: const BoxDecoration(
                gradient: LinearGradient(
                  ...
                  colors: [Color(0xFF050D1A), Color(0xFF0A1628), Color(0xFF060E1C)],
                  stops: [0.0, 0.4, 1.0],
                ),
              ),
            ),
          ...
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                gradient: RadialGradient(colors: [_blue.withValues(alpha: 0.22), Colors.transparent]),
              ),
            ),
          ),
          // Orbe cyan en bas
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                gradient: RadialGradient(colors: [_cyan.withValues(alpha: 0.14), Colors.transparent]),
              ),
            ),
          ),
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(color: _blue.withValues(alpha: 0.5), blurRadius: 32, offset: const Offset(0, 10)),
                            ],
                          ),
                          child: const Center(
                            child: Text('B',
                                style: TextStyle(fontSize: 40, fontWeight: FontWeight.w900, color: Colors.white, height: 1)),
                          ),
                        ),
                        ...
                        Text('Services à domicile · Côte d\'Ivoire',
                            style: TextStyle(fontSize: 12, color: Colors.white.withValues(alpha: 0.45), letterSpacing: 0.5)),
                      ],
                    ),
                  ),
                              ...
                              child: Container(
                                ...
                                child: TabBar(
                                  ...
                                  indicator: BoxDecoration(
                                    gradient: const LinearGradient(colors: [_blue, _blueDeep]),
                                    borderRadius: BorderRadius.circular(12),
                                    boxShadow: [BoxShadow(color: _blue.withValues(alpha: 0.4), blurRadius: 12)],
                                  ),
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _SocialButton(
                          label: 'Google',
                          iconAsset: 'assets/illustrations/icons/brand_google.svg',
                      ...
                      Expanded(
                        child: _SocialButton(
                          label: 'Apple',
                          iconAsset: 'assets/illustrations/icons/brand_apple.svg',
  ...
  Widget _buildLoginForm() {
    return Column(
      ...
      children: [
        _PremiumField(controller: emailCtrl, label: 'Adresse email', icon: Icons.email_outlined, inputType: TextInputType.emailAddress),
        const SizedBox(height: 14),
        _PremiumField(
          controller: passCtrl,
          label: 'Mot de passe',
        ...
        Align(
          ...
          child: TextButton(
            ...
            style: TextButton.styleFrom(foregroundColor: _blue, padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 8)),
            child: const Text('Mot de passe oublié ?', style: TextStyle(fontSize: 13)),
          ),
        ),
        const SizedBox(height: 6),
        _GradientButton(label: 'Se connecter', loading: _loading, onPressed: _doLogin),
      ],
    );
  }
  ...
  Widget _buildRegisterForm() {
    return Column(
      ...
      children: [
        _PremiumField(
          controller: nameCtrl, label: 'Nom complet', icon: Icons.person_outline_rounded,
          capWords: true,
        ),
        const SizedBox(height: 14),
        ...
        Container(
          ...
          child: IntlPhoneField(
            ...
            dropdownIcon: Icon(Icons.arrow_drop_down, color: Colors.white.withValues(alpha: 0.4)),
        ...
        const SizedBox(height: 14),
        _PremiumField(
          controller: passCtrl, label: 'Mot de passe', icon: Icons.lock_outline_rounded,
          obscure: hidden, onToggleObscure: () => setState(() => hidden = !hidden),
        ),
        const SizedBox(height: 14),
        _PremiumField(
          controller: pass2Ctrl, label: 'Confirmer le mot de passe', icon: Icons.lock_outline_rounded,
          obscure: hidden2, onToggleObscure: () => setState(() => hidden2 = !hidden2),
          onSubmitted: (_) => _doRegister(),
        ),
        const SizedBox(height: 20),
        _GradientButton(label: 'Créer mon compte', loading: _loading, onPressed: _doRegister),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: TextField(
        ...
        textCapitalization: capWords ? TextCapitalization.words : TextCapitalization.none,
        style: const TextStyle(color: Colors.white, fontSize: 15),
        decoration: InputDecoration(
          labelText: label,
          labelStyle: TextStyle(color: Colors.white.withValues(alpha: 0.45), fontSize: 14),
          floatingLabelStyle: const TextStyle(color: Color(0xFF2563EB), fontSize: 12),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: loading
              ? []
              : [BoxShadow(color: const Color(0xFF2563EB).withValues(alpha: 0.45), blurRadius: 20, offset: const Offset(0, 8))],
        ),
        child: Center(
          child: loading
...
class _SocialButton extends StatelessWidget {
  const _SocialButton({required this.label, required this.iconAsset, required this.onPressed});
