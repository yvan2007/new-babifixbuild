...
def home(request):
            ...
            if request.POST.get('website', '').strip():
                ...
                if count >= _CONTACT_RATE_LIMIT:
                    ...
                    if name and c_email and subject and body:
                        ...
                        contact_message = 'Veuillez remplir tous les champs du formulaire.'
        else:
            email = request.POST.get('email', '').strip()
            if email:
                _, created = Lead.objects.get_or_create(email=email)
                message = 'Merci, votre demande a bien ete enregistree.' if created else 'Cet email est deja inscrit.'
            else:
    ...
    faq_items = [{'question': f.question, 'answer': f.answer} for f in faq_db] if faq_db else faq_items
    ...
    context = {
        ...
        'stats': stats_list,
        'features': [
            'Sécurisé', 'Local', 'Simple', 'Mobile Money', 'Rapide', 'Qualité garantie'
        ],
        ...
        'admin_api_base': os.getenv('BABIFIX_ADMIN_API_BASE', 'http://127.0.0.1:8002').rstrip('/'),
        'faq_items': faq_items,
    }
...
def telecharger_app_client(request):
    ...
    lines = []
    if ios:
        lines.append(f'<p><a class="btn primary" href="{ios}" rel="noopener">App Store (iOS)</a></p>')
    if android:
        lines.append(f'<p><a class="btn primary" href="{android}" rel="noopener">Google Play (Android)</a></p>')
    if not lines:
        lines.append('<p>Les liens stores seront affichés ici une fois renseignés dans l’admin BABIFIX (SiteContent).</p>')
    return render(
        request,
...
def devenir_prestataire(request):
    ...
    lines = []
    if ios:
        lines.append(f'<p><a class="btn primary" href="{ios}" rel="noopener">App Prestataire — App Store</a></p>')
    if android:
        lines.append(f'<p><a class="btn primary" href="{android}" rel="noopener">App Prestataire — Google Play</a></p>')
    if not lines:
        lines.append('<p>Téléchargez l’application prestataire BABIFIX depuis les stores (liens à configurer dans l’admin).</p>')
    return render(
        request,
        'vitrine/simple_page.html',
...
def creer_un_compte(request):
    return render(request, 'vitrine/simple_page.html', {'title': 'Creer un compte', 'body': 'La creation de compte client sera activee sur le backend central BABIFIX.'})
...
def api_newsletter_subscribe(request):
    ...
    lead, created = Lead.objects.get_or_create(
        email=email,
        defaults={'name': name, 'role': role} if name else {}
    )
    ...
    return JsonResponse({
        'ok': True,
        'message': 'Inscription confirmée' if created else 'Cet email est déjà inscrit.',
    })
