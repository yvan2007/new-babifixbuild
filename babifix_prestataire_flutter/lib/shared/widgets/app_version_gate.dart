/// Contrôle de version minimale (force update) — app prestataire.
/// Compare la version installée à la version minimale renvoyée par le backend
/// (`/api/app/version?app=prestataire`). Si trop ancienne → écran bloquant.
/// Tolérant : silencieux en cas d'erreur réseau.
import 'dart:convert';
...
Future<void> checkAppVersionGate(
  ...
  try {
    ...
    final msg = (j['message'] ?? 'Mettez à jour BABIFIX pour continuer.')
        .toString();
